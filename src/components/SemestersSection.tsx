import { SEMESTERS } from '../constants'; // Pull layout from the new constants file
import { useCourseStore } from '../store/courses'; // Pull DB data from store
    ...
    <section id="semesters" className="py-24">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {SEMESTERS.map(sem => {
            const semCourses = sem.subjects.map(id => courses.find(c => c.id === id)).filter(Boolean);
            const theory = semCourses.filter(c => !c!.isLab);
            const labs   = semCourses.filter(c => c!.isLab);
            return (
              <div key={sem.num} className="bg-surface border border-border rounded-2xl p-6 hover:border-accent/50 transition-colors">
                ...
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-muted text-sm">{sem.label} · {semCourses.length} courses</p>
                  </div>
                </div>
